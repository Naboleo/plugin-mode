					...
					if (is_array($options)) {
						foreach ($options as $key => $value) {
							if($key == 'enable'){
								...
								if($key == 'scenario_id'){
									$txtscen=scenario::byId($value)->getName();
									log::add('mode', 'debug',$txtact.' -> [Nom=#'.$txtscen.'#]');
								}elseif ($key == 'cmd_id'){
									$txtcmd=cmd::byId($value)->getName();
									log::add('mode', 'debug',$txtact.' -> [Nom=#'.$txtcmd.'#]');
								}else{
									log::add('mode', 'debug',$txtact.' -> ['.$key.'='.evaluate($value).']');
								}
							}
						}
					}
